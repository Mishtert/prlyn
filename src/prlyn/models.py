...
class PositionScore(BaseModel):
    ...
    end_density: float
    buried_instructions: List[int] = Field(description="Indices of instructions in the middle zone")
    score: float
...
class VulnerabilityScore(BaseModel):
    vulnerability_zones: List[int] = Field(description="Sentence indices with vague/unconstrained context")
...
class FlowScore(BaseModel):
    sentence_similarities: List[float] = Field(description="Cosine similarity between adjacent sentences")
...
class StrengthScore(BaseModel):
    ...
    weak_instructions: List[int] = Field(description="Indices of instructions with weak verbs (can, would)")
