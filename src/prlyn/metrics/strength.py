...
WEAK_VERBS = {"can", "may", "might", "could", "try", "possibly", "optionally", "probably"}

class StrengthAnalyzer:
    def analyze(self, sentences: List[ClassifiedSentence]) -> StrengthScore:
        instructions = [s for s in sentences if s.primary_label == SentenceLabel.INSTRUCTION]
        if not instructions:
            return StrengthScore(imperative_density=0.0, weighted_strength_score=0.0, weak_instructions=[])
