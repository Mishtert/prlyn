...
@mcp.tool()
def analyze_prompt(prompt: str, output_format: str = "json", model_name: Optional[str] = None) -> str:
    """
...
def run() -> None:
    ...
    parser.add_argument("prompt", nargs="?", help="Prompt text to analyze (skips server mode if provided)")
    ...
    if args.prompt:
        ...
        if args.diff:
            ...
            if prev:
                ...
                c_read = result.readability_score.flesch_reading_ease if result.readability_score else 0
                print(f"Readability: {p_read} -> {c_read} ({'+' if c_read > p_read else ''}{c_read - p_read:.2f})")
