...
def generate_improvement_template(analysis: AnalysisResult) -> str:
    ...
    if analysis.position_score and analysis.position_score.buried_instructions:
        ...
        sections.append("The following instructions are buried in the middle of the prompt.")
        sections.append("Move them to the START (for critical setup) or END (for final reminders):\n")
        for idx in analysis.position_score.buried_instructions:
            if idx < len(analysis.sentences):
                sent = analysis.sentences[idx]
                sections.append(f"- **Sentence {idx + 1}**: \"{sent.text[:80]}...\" → Move to START or END.")
        sections.append("")
    ...
    if analysis.ambiguity_score and analysis.ambiguity_score.vague_terms_found:
        ...
        sections.append("Replace the following vague terms with specific, concrete alternatives:\n")
    ...
    if analysis.ambiguity_score and analysis.ambiguity_score.hedging_terms_found:
        ...
        for term in analysis.ambiguity_score.hedging_terms_found:
            sections.append(f"- Remove or strengthen \"{term}\" (e.g., 'maybe' → remove, 'try to' → 'always').")
        sections.append("")
    ...
    if analysis.strength_score and analysis.strength_score.weak_instructions:
        ...
        for idx in analysis.strength_score.weak_instructions:
            if idx < len(analysis.sentences):
                sent = analysis.sentences[idx]
                sections.append(f"- **Sentence {idx + 1}**: \"{sent.text[:80]}...\" → Use 'must', 'always', or 'ensure'.")
        sections.append("")
    
    # 5. Missing Delimiters
    if analysis.vulnerability_score and not analysis.vulnerability_score.has_strong_delimiters:
        sections.append("### 5. Missing Delimiters (Add Separation)\n")
        sections.append("Add clear delimiters (e.g., ```xml, <user_input>, ---) to separate:\n")
    ...
    if analysis.flow_score and analysis.flow_score.disjointed_indices:
        ...
        sections.append("The following sentences have low semantic connection to their neighbors.\n")
        sections.append("Add transitional phrases or reorder for better logical flow:\n")
    ...
    if analysis.contradictions:
        ...
        sections.append("The following sentence pairs may contradict each other. Resolve or clarify:\n")
        for pair in analysis.contradictions[:3]:  # Limit to 3
            sections.append(f"- **Sentence {pair.sentence_a_index + 1}** vs **Sentence {pair.sentence_b_index + 1}**:")
