...
class TestPrlynE2E(unittest.TestCase):
    ...
    def test_001_core_framework(self) -> None:
        ...
        # Check Placeholders
        self.assertTrue(any(p.type.name == "CODE_BLOCK" for p in result.placeholders.values()))
        self.assertTrue(any(p.type.name == "QUOTED_STRING" for p in result.placeholders.values()))
        
        # Check Classification
        instructions = [s for s in result.sentences if s.primary_label == SentenceLabel.INSTRUCTION]
    ...
    def test_003_advanced_metrics(self) -> None:
        ...
        prompt = "This is filler context. " * 5 + "Create a summary of this text immediately. " + "This is filler context. " * 5
        result = self.analyzer.analyze(prompt)
        ...
        found_buried = len(result.position_score.buried_instructions) > 0
        self.assertTrue(found_buried, "Should detect buried instruction 'Create a summary...'")
    ...
    def test_004_complex_analytics(self) -> None:
        ...
        if result.redundancy_score:
            self.assertTrue(len(result.redundancy_score.clusters) > 0, "Should detect redundancy between concise and brief")
