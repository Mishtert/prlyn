...
import pytest
from prlyn.models import AnalysisResult, SentenceLabel, ClassifiedSentence, Placeholder, PositionScore, AmbiguityScore, StrengthScore, VulnerabilityScore
...
def test_generate_template_with_issues():
    ...
    analysis = AnalysisResult(
        raw_text="Try to do stuff.",
        clean_text="Try to do stuff.",
        sentences=[ClassifiedSentence(text="Try to do stuff.", start_char=0, end_char=16, primary_label=SentenceLabel.INSTRUCTION)],
        ambiguity_score=AmbiguityScore(
            vague_term_density=0.1, unresolved_coref_score=0, hedging_density=0.1,
            vague_terms_found=["stuff"], hedging_terms_found=["try to"]
        ),
        position_score=PositionScore(
            start_density=0, middle_density=1, end_density=0, score=0.5,
